...
class F1:
  ...
  def compute_frequency(self, gts):
    freq = dict()
    with open('../../datasets/bladder/data_organized_5fold2/bladder_vocab.txt','r') as f:
    ...
    for i in weight.keys():
      if freq[i] == 0:
        ...
        # weight[i] = np.log(self.freq_norm) / max(np.log(freq[i]), 1)
        weight[i] = freq_norm / max(freq[i], 1)
    return weight
  ...
  def compute_score(self, gts, res):
    ...
    for id in imgIds:
      ...
      # assert(len(ref) > 0)
      weight = self.compute_frequency(gts[id])
      
      hypo = res[id][0]
      f1 = 0
      for i in range(len(gts[id])):
        ref  = gts[id][i] # we have multiple references
        f1 += self.get_score(hypo, ref, weight)
  ...
  def get_score(self, hypo, ref, weight):
    ...
    try:
      for gram in hypo:
        if gram in ref:
          basic_scores['tp'] += weight[gram] # true positive
        else:
          basic_scores['fp'] += weight[gram] # false positive
      for gram in ref:
        if gram not in hypo:
          basic_scores['fn'] += weight[gram] # false negative
    except Exception, e:
      print (e)
      print (gram)
